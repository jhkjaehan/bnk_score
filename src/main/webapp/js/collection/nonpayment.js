...
function loadMstrSearchList(data) {
    ...
    list.forEach((row, index) => {
        html += '<tr class="hover:bg-gray-50 text-xs cursor-pointer" data-index="' + index + '" data-call-id="' + escapeHtml(row.callId) + '">'
            ...
            + '<td class="px-4 py-2 whitespace-nowrap text-center">' + escapeHtml(row.item07) + '</td>'
            + '<td class="px-4 py-2 whitespace-nowrap text-center">' + escapeHtml(row.item06) + '</td>'
            + '<td class="px-4 py-2 whitespace-nowrap text-center">' + escapeHtml(row.item08) + '</td>'
            + '</tr>';
    });
...
function bindEvent() {
    ...
    // 이벤트 위임: 테이블 행 클릭 (행마다 이벤트 바인딩 대신 tbody에 한 번만 바인딩)
    $('#mstrListTable tbody').on('click', 'tr', function(e) {
        ...
        if ($(e.target).hasClass('call-number-link') || $(e.target).closest('.call-number-cell').length > 0) {
            openDetailPage($(this).data('call-id'));
        }
    });
