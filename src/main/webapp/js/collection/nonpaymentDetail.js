...
function downloadDetail() {
    ...
    $.ajax({
        ...
        success: function(blob) {
            ...
            a.href = url;
            a.download = `미납안내_${data.basicInfo.callId}.xlsx`;
...
function selectMstrScore() {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: {taskId : "TA0001", callId : callId},
        success: function(response) {
...
function loadMstrScore(data) {
    ...
    const template =
        ...
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-blue-600 font-bold\" data-scoreid=\"S0001\" data-field=\"totalScore\"></td>" +
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900\" data-scoreid=\"S0002\"></td>" +
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900\" data-scoreid=\"S0003\"></td>" +
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900\" data-scoreid=\"S0004\"></td>" +
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900\" data-scoreid=\"S0005\"></td>" +
        "   <td class=\"px-6 py-4 whitespace-nowrap text-sm text-center text-red-600\" data-scoreid=\"S0006\"></td>" +
        "</tr>";

    $("#scoreTable tbody tr").not(':first').remove();

    let dataRow = $(template);

    data.forEach(col => {
        dataRow.find("td").filter(function () { return $(this).data("scoreid") === col.scoreId;}).text(col.scoreValue);
    })
